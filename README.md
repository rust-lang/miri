...
* `-Zmiri-provenance-gc=<blocks>` configures how often the pointer provenance garbage collector runs.
  ...
  this to `0` disables the garbage collector, including `tree-gc` listed below, which causes some programs
  to have explosive memory usage and/or super-linear runtime.
* `-Zmiri-tree-gc-visits=<visits>` sets the interval (in visited tree nodes) of the provenance
  garbage collector under Tree Borrows. The default is `20000`. Setting this to `0` disables the 
  visit-based garbage collector, falling back to the basic block interval of `-Zmiri-provenance-gc`.
