...
fn main() {
    ...
    let t = thread::spawn(move || {
        ...
        fn retag(_x: &mut i32) {}
        retag(unsafe { &mut *ptr.0 }); //~ERROR: Data race detected between (1) non-atomic read on thread `main` and (2) implicit write from the aliasing model of type `i32` on thread `unnamed-1`
    });
