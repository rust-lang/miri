...
fn thread1_inner(x: &mut u8, comm: &Arc<AtomicI32>) {
    ...
    if cfg!(access) {
        let y = *x;
        miriwrite(&format!("Thread 1: pretend read -> {y}!\n"));
    }
    miriwrite("Thread 1: pretend read just happened!\n");
}
...
fn thread2(x: SendPtr, comm: Arc<AtomicI32>) {
    ...
    miriwrite("Thread 2: about to store non-atomically!\n");
    unsafe { *x = 42 }; //~ ERROR: /(Data race detected between \(1\) non-atomic read on thread `unnamed-[0-9]+` and \(2\) non-atomic write on thread `unnamed-[0-9]+`|write access through .* is forbidden)/
