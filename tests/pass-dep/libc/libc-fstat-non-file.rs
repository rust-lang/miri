...
fn test_fstat_stdout() {
    ...
    // In this test setup, stdout is expected to be a character device or a pipe.
    let file_type = stat.st_mode & libc::S_IFMT;
    assert!(
        file_type == libc::S_IFCHR || file_type == libc::S_IFIFO,
        "stdout should be S_IFCHR or S_IFIFO, got {:#o}",
        file_type
    );
...
fn test_fstat_stderr() {
    ...
    // In this test setup, stderr is expected to be a character device or a pipe.
    let file_type = stat.st_mode & libc::S_IFMT;
    assert!(
        file_type == libc::S_IFCHR || file_type == libc::S_IFIFO,
        "stderr should be S_IFCHR or S_IFIFO, got {:#o}",
        file_type
    );
