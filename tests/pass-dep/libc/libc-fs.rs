...
fn main() {
    ...
    test_read_atomic_contiguous();
    test_read_atomic_scattered();
}
...
/// Tests concurrent atomic reads with a contiguous buffer.
/// Validates that multiple threads can safely read from the same file
/// using a single continuous memory region for each read operation.
fn test_read_atomic_contiguous() {
    // Create test file with known content
    let content = b"The quick brown fox jumps over the lazy dog";
    let path = utils::prepare_with_content("miri_test_atomic_read_contiguous.txt", content);

    // Convert path to a string that can be safely cloned for each thread
    let path_string = path.into_os_string().into_string().unwrap();

    // Spawn multiple threads to read concurrently
    let mut handles = Vec::new();
    let thread_count = 4;
    let read_size = 8; // Each thread reads 8 bytes

    for thread_id in 0..thread_count {
        // Clone the path string for this thread
        let thread_path = path_string.clone();

        let handle = std::thread::spawn(move || {
            // Create CString inside the thread
            let name = CString::new(thread_path).unwrap();
            let name_ptr = name.as_ptr();

            unsafe {
                // Open file descriptor for this thread
                let fd = libc::open(name_ptr, libc::O_RDONLY);
                assert!(fd >= 0, "Failed to open file in thread {}", thread_id);

                // Seek to thread-specific offset
                let offset = thread_id * read_size;
                assert_eq!(
                    libc::lseek(fd, offset as i64, libc::SEEK_SET),
                    offset as i64,
                    "Failed to seek in thread {}",
                    thread_id
                );
                ...
                let bytes_read =
                    libc::read(fd, buffer.as_mut_ptr() as *mut libc::c_void, read_size as usize);

                assert_eq!(
                    bytes_read as usize, read_size,
                    "Incorrect read size in thread {}",
                    thread_id
                );
                ...
                assert_eq!(
                    &buffer,
                    &content[offset..offset + read_size],
                    "Incorrect data read in thread {}",
                    thread_id
                );
...
/// Tests concurrent atomic reads with scattered buffers.
/// Validates that multiple threads can safely read from the same file
/// using multiple discontinuous memory regions for each read operation.
fn test_read_atomic_scattered() {
    // Create test file with repeating pattern for easy verification
    let pattern = b"ABCDEFGH";
    let repeat_count = 4;
    let test_content: Vec<u8> =
        pattern.iter().cycle().take(pattern.len() * repeat_count).copied().collect();
    let path = utils::prepare_with_content("miri_test_atomic_read_scattered.txt", &test_content);

    // Convert path to a string that can be safely cloned for each thread
    let path_string = path.into_os_string().into_string().unwrap();

    // Spawn threads for concurrent scattered reads
    let mut handles = Vec::new();
    let thread_count = 2;
    ...
    for thread_id in 0..thread_count {
        ...
        let handle = std::thread::spawn(move || {
            // Convert path to C string for libc
            let name = CString::new(thread_path).unwrap();
            let name_ptr = name.as_ptr();
            ...
            unsafe {
                ...
                // Create scattered buffers: two 4-byte regions
                ...
                // Seek to thread-specific offset
                let offset = thread_id * 8; // Each thread reads 8 bytes total
                assert_eq!(
                    libc::lseek(fd, offset as i64, libc::SEEK_SET),
                    offset as i64,
                    "Failed to seek in thread {}",
                ...
                // Perform scattered read
                let bytes_read = libc::readv(fd, iov.as_ptr(), 2);
                assert_eq!(
                    bytes_read as usize, 8,
                    "Incorrect scattered read size in thread {}",
                    thread_id
                );
                ...
                assert_eq!(
                    ...
                    "Incorrect data in first buffer of thread {}",
                    thread_id
                );
                assert_eq!(
                    &buffer2,
                    &content[offset + 4..offset + 8],
                    "Incorrect data in second buffer of thread {}",
                    thread_id
                );
