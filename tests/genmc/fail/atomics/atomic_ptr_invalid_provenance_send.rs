...
fn miri_start(_argc: isize, _argv: *const *const u8) -> isize {
    unsafe {
        ...
        // Create a pointer with the provenance of `b`, but the address of `a`.
        let fake_a_ptr = b_ptr.wrapping_byte_add(a_ptr as usize).wrapping_byte_sub(b_ptr as usize);
        if a_ptr as usize != fake_a_ptr as usize {
            std::process::abort();
        }
