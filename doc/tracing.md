...
### Good code snippets to analyze traces of

This code snippet repeats the same operation many times, and is thus ideal when you want to analyze trace files to understand which parts of Miri are slow, since the one-time-only operations do not impact the statistics too much if `n` is big enough.
...
You can generate a trace file out of it with:

```sh
# in Miri
MIRI_TRACING=1 ./miri run --features=tracing ../benchmark.rs -- 100
# in rustc (remember to also change bootstrap.toml)
MIRI_TRACING=1 ./x.py run miri --stage 1 --args ../benchmark.rs --args "--" --args 100
```

And here is another program that does the same thing but on multiple threads:
