...
impl IsolatedAlloc {
    ...
    unsafe fn dealloc_huge(&mut self, ptr: *mut u8, layout: Layout) {
        ...
        let idx = self
            .huge_ptrs
            .iter()
            .position(|pg| ptr.addr() == pg.0.addr().next_multiple_of(layout.align()))
            .expect("Freeing unallocated pages");
        // And kick it from the list
        let (un_offset_ptr, _) = self.huge_ptrs.remove(idx);
        // SAFETY: huge_ptrs contains allocations that we own, and huge_normalized_layout
        // computed the size of the original allocation (if the user gave us the right `layout`,
        // which we may assume).
        unsafe {
