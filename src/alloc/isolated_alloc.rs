...
impl IsolatedAlloc {
    ...
    /// For greater-than-page-sized allocations, returns the minimum allocation
    /// size to make sure the alignment requested can be given.
    #[inline]
    fn huge_normalized_layout(&self, layout: Layout, page_size: usize) -> usize {
        // Allocate in page-sized chunks
        let size = layout.size().next_multiple_of(page_size);
        let pg_count = size.div_ceil(page_size);
        // And make sure the align is at least one page
        let align = std::cmp::max(layout.align(), page_size);
        let extra_pages = align.strict_div(page_size).saturating_sub(1);

        pg_count.strict_add(extra_pages).strict_mul(page_size)
    }
    ...
    fn add_page(&mut self) -> (*mut u8, &mut DenseBitSet<usize>) {
        // SAFETY: mmap is safe to call as long as the length is a multiple of
    ...
    unsafe fn alloc_huge(&mut self, layout: Layout) -> *mut u8 {
        let size = self.huge_normalized_layout(layout, self.page_size);
        // SAFETY: Layout size being a multiple of the system pagesize is enforced
        // by huge_normalized_layout
        let ret = unsafe {
            libc::mmap(
                std::ptr::null_mut(),
                size,
        ...
        // Align the pointer as requested
        let ofs = ret.addr().next_multiple_of(layout.align()).strict_sub(ret.addr());
        self.huge_ptrs.push((ret, layout.size()));
        // SAFETY: huge_normalized_layout ensures that we've overallocated
        // enough space for this to be valid
        unsafe { ret.add(ofs) }
    }
    ...
    pub unsafe fn dealloc(&mut self, ptr: *mut u8, layout: Layout) {
        ...
        if self.is_huge_alloc(&layout) {
            ...
            if self.page_infos[idx].is_empty() {
                ...
                // with this layout whose size is the system page size
                unsafe {
    ...
    unsafe fn dealloc_huge(&mut self, ptr: *mut u8, layout: Layout) {
        ...
        let (un_offset_ptr, _) = self.huge_ptrs.remove(idx);
        // SAFETY: huge_normalized_layout ensures the validity of the size, and
        // we know the pointer was the original page-aligned one
        unsafe {
            assert_eq!(libc::munmap(un_offset_ptr.cast(), size), 0);
        }
    }
}
