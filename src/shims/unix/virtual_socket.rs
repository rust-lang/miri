...
impl FileDescription for VirtualSocket {
    ...
    fn destroy<'tcx>(
        ...
        if let Some(peer_fd) = self.peer_fd().upgrade() {
            ...
            ecx.notify_fd_readiness(peer_fd, /* force_edge */ false)?;
        }
        interp_ok(Ok(()))
    }
    ...
    fn readiness<'tcx>(&self) -> InterpResult<'tcx, Readiness> {
        ...
        // If other readiness events need to be supported in the future, the check should be added here.
        ...
        if let Some(peer_fd) = self.peer_fd().upgrade() {
            ...
            readiness.readable = true;
            readiness.writable = true;
            // If there is data lost in peer_fd, the error readiness is set.
...
fn virtual_socket_write<'tcx>(
    ...
    if available_space == 0 {
        ...
        // Notify all readiness consumers that the socket might no longer be writable
        // and that the peer might now be readable. The readiness event of the peer
        // seems to be always sent on Linux -- even when the FD was readable before.
        ecx.notify_fd_readiness(self_ref, /* force_edge */ false)?;
        ecx.notify_fd_readiness(peer_fd, /* force_edge */ true)?;
...
fn virtual_socket_read<'tcx>(
    ...
    if readbuf.borrow_mut().buf.is_empty() {
        ...
        if let Some(peer_fd) = self_ref.peer_fd().upgrade() {
            ...
            // Notify all readiness consumers that the peer is now writable.
            // Linux seems to always emit an edge when the read buffer is now empty.
            // (Linux also does that if this was a "big" read, but to avoid some arbitrary
            // threshold, we do not match that.)
            ecx.notify_fd_readiness(peer_fd, /* force_edge */ readbuf_now_empty)?;
        };
        // Notify all readiness consumers that the socket might no longer be readable.
        ecx.notify_fd_readiness(self_ref, /* force_edge */ false)?;
