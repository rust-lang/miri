...
impl FileDescription for EventFd {
    ...
    fn readiness<'tcx>(&self) -> InterpResult<'tcx, Readiness> {
        // We only check the readable and writable readiness for eventfd. If other readiness
        // events need to be supported in the future, the check should be added here.
...
fn eventfd_write<'tcx>(
    ...
    match eventfd.counter.get().checked_add(num) {
        Some(new_count @ 0..=MAX_COUNTER) => {
            ...
            ecx.notify_fd_readiness(eventfd, /* force_edge */ true)?;
