...
impl Permission {
    ...
    /// Like `can_be_replaced_by_child`, but sound even when `child` has *siblings* under `self`,
    /// i.e. when compacting a node that has more than one child (and reparenting all of them onto
    /// the grandparent).
    ///
