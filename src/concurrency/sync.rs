...
pub trait EvalContextExt<'tcx>: crate::MiriInterpCxExt<'tcx> {
    ...
    fn rwlock_reader_unlock(&mut self, rwlock_ref: &RwLockRef) -> InterpResult<'tcx, bool> {
        ...
        if this.rwlock_is_locked(rwlock_ref).not() {
            ...
            let mut rwlock = rwlock_ref.0.borrow_mut();
            let vclock = &rwlock.clock_current_readers.clone();
            // TODO: clone needed here?
            rwlock.clock_unlocked.clone_from(vclock);
    ...
    fn rwlock_writer_unlock(&mut self, rwlock_ref: &RwLockRef) -> InterpResult<'tcx, bool> {
        let this = self.eval_context_mut();
        let thread = this.active_thread();
        let (writer_to_unblock, readers_to_unblock) = {
            let mut rwlock = rwlock_ref.0.borrow_mut();
            if let Some(current_writer) = rwlock.writer {
                ...
                // starve readers.
                if let Some(writer) = rwlock.writer_queue.pop_front() {
                    (Some(writer), VecDeque::new())
                } else {
                    (None, std::mem::take(&mut rwlock.reader_queue))
                }
            } else {
                return interp_ok(false);
            }
        ...
        // We have no mutable borrow of `rwlock` anymore, so do the actual unblocking,
        // which can trigger callbacks that use the `rwlock`.
        if let Some(writer) = writer_to_unblock {
            this.unblock_thread(writer, BlockReason::RwLock)?;
        } else {
