...
pub trait EvalContextExt<'tcx>: crate::MiriInterpCxExt<'tcx> {
    ...
    fn join_thread(
        ...
        let this = self.eval_context_mut();
        if this.machine.threads.threads[joined_thread_id].join_status == ThreadJoinStatus::Detached
        {
        ...
        this.machine.threads.threads[joined_thread_id].join_status = ThreadJoinStatus::Joined;
        if !this.machine.threads.threads[joined_thread_id].state.is_terminated() {
            trace!(
                "{:?} blocked on {:?} when trying to join",
                this.machine.threads.active_thread, joined_thread_id
            );
            ...
            this.machine.threads.block_thread(
    ...
    fn join_thread_exclusive(
        ...
        let this = self.eval_context_mut();
        if this.machine.threads.threads[joined_thread_id].join_status == ThreadJoinStatus::Joined {
        ...
        self.join_thread(joined_thread_id, success_retval, return_dest)
    }
