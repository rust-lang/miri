...
impl GenmcConfig {
    ...
    pub fn parse_arg(
        ...
        } else if let Some(trimmed_arg) = trimmed_arg.strip_prefix("schedule-policy=") {
            genmc_config.params.schedule_policy = trimmed_arg.parse().map_err(|_err| {
                format!("-Zmiri-genmc-schedule-policy={{wf, wfr, ltr, arbitrary}}")
            })?;
