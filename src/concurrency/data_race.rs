...
pub enum NaReadType {
    ...
    // An implicit read generated by a retag (or protector end)
    Retag,
}
...
pub enum NaWriteType {
    ...
    // An implicit write generated by a retag (or protector end)
    Retag,
...
impl GlobalState {
    ...
    /// Like `release_clock` but also sets the type of the new read to NaRead::Retag.
    pub fn release_clock_retag<'tcx, R>(
        &self,
        threads: &ThreadManager<'tcx>,
        callback: impl FnOnce(&VClock) -> R,
    ) -> R {
        ...
        // occurred before the release. Crucially, the callback is invoked on the *old* clock!
        clocks.increment_clock(index, span);
        clocks.clock.index_mut(index).set_read_type(NaReadType::Retag);
    ...
    /// Returns a vector clock that is a lower bound of all threads.
    /// In other words, if something happens-before the result of this method,
    /// it will have happened-before all threads and is therefore "globally observed."
    /// Useful for garbage collecting.
    /// Returns `None` when there are no active threads.
    pub(crate) fn all_threads_lower_bound(&self) -> Option<VClock> {
        let mut res = None;
        for (tidx, vc) in self.vector_clocks.borrow().iter_enumerated() {
            // We do not care about terminated threads
            if self.reuse_candidates.borrow().contains(&tidx) {
                continue;
            }
            res.get_or_insert_with(|| vc.clock.clone()).meet(&vc.clock);
        }
        res
    }
}
